  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        toast.success(t("login.confirm_email"));
      } else {
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-black/[0.06] border border-border/50 p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-muted-foreground mt-1.5">
              {isSignUp ? t("login.sub_signup") : t("login.sub_signin")}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            {/* Email — icon on text-start side via logical properties */}
            <div className="relative">
              <Mail className="absolute start-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none" />
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder={t("login.email")}
                className="w-full rounded-xl border border-input bg-background ps-11 pe-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent transition-all duration-200"
                required
              />
            </div>

            {/* Password — icon on start, eye toggle on end */}
            <div className="relative">
              <Lock className="absolute start-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none" />
              <input
                type={showPassword ? "text" : "password"}
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                placeholder={t("login.password")}
                className="w-full rounded-xl border border-input bg-background ps-11 pe-11 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent transition-all duration-200"
                required
                minLength={6}
              />
              <button
